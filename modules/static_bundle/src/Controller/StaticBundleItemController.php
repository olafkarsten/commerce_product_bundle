 ...
 */
class StaticBundleItemController extends ControllerBase implements ContainerInjectionInterface {
...
//  }
  ...
  public function revisionShow($revision) {
    $item_storage = $this->entityManager()->getStorage('commerce_static_bundle_item');
    if ($item = $item_storage->loadRevision($revision)) {
      $item = $this->entityManager()->getTranslationFromContext($item);
      $item_view_controller = new StaticBundleItemViewController($this->entityManager, $this->renderer, $this->currentUser());
  ...
  public function revisionPageTitle($revision) {
    $bundle_item = $this->entityManager()->getStorage('commerce_static_bundle_item')->loadRevision($revision);
    ...
    foreach ($this->getRevisionIds($item, $bundle_storage) as $vid) {
      ...
      if ($revision->hasTranslation($langcode) && $revision->getTranslation($langcode)->isRevisionTranslationAffected()) {
        ...
        if ($vid != $item->getRevisionId()) {
          $link = $this->l($date, new Url('entity.commerce_static_bundle_item.revision', ['commerce_static_bundle_item' => $item->id(), 'revision' => $vid]));
        }
        else {
          $link = $item->link($date);
        }
