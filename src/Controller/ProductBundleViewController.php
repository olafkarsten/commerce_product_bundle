 ...
 */
class ProductBundleViewController extends EntityViewController {
   ...
   * {@inheritdoc}
   */
  public function view(EntityInterface $bundle, $view_mode = 'full', $langcode = NULL) {
    $build = parent::view($bundle, $view_mode, $langcode);

    foreach ($bundle->uriRelationships() as $rel) {
      $url = $bundle->toUrl($rel);
   ...
   *   The page title.
   */
  public function title(EntityInterface $item) {
    return $this->entityManager->getTranslationFromContext($item)->label();
  }
