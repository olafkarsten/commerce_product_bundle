 ...
 */
class ProductBundleItemViewController extends EntityViewController {
   ...
   * {@inheritdoc}
   */
  public function view(EntityInterface $item, $view_mode = 'full', $langcode = NULL) {
    $build = parent::view($item, $view_mode, $langcode);

    foreach ($item->uriRelationships() as $rel) {
      $url = $item->toUrl($rel);
